...
\end{align*}
Notice that if $(m,g)\sim(n,g)$ then $m=n$ is provable in type theory. 
...
Finite multisets can be viewed as a generalization of unordered pairs and again, the construction of a definable quotient over the setoids of unordered pairs $(\List A,\sim)$ as defined in Section~~\ref{sec:setoids:examples} depends on the choice of $A$. 

We define $Q = \set{(m , s) \mid  \forall i,j:\Fin\,m, i\leq j\implies s\,i \leq s\,j}$ and
\[ [(m,s)] = (m,\order_m.\]
Clearly $[(a,b)] \sim (a,b)$ and if $a\leq b$ then $[(a,b)]=(a,b)$. Both facts follow from
 the properties of $\min$ and $\max$. We consider two cases:
...
\begin{description}
\item[$A=\N\to\N$] \hfill\\
First we define a family of orders $\set{{\leq_n}}_{n:\N}$ 
on sequences of natural numbers where $f\leq_n g$ 
if and only if $[f_0,\dots, f_n]$ comes before $[g_0,\dots, g_n]$ in the lexicographic order: 


\begin{align*}
&{{-}\leq_{-}{-}} : \N\to (\N\to\N) \to (\N\to\N)\to \Bool\\
&{f\leq_n g} =  f\,i\leq g\,i \\
&\text{\phantom{$f\leq_n g =$} where }i =\min\set{i:\N\mid i>n\lor f\,i\neq g\,i}.
\end{align*}
Notice that if $f<_n g$ for some $n$ then $f<_m g$ for all $m$ greater than $n$. 

Now, given a finite set of natural sequences $\varphi:\Fin m\to (\N\to\N)$, we can order it using any algorithm
\[\order_{m,n} :(\Fin m \to (\N\to\N)) \to (\Fin m \to (\N\to\N))\] 
which sorts $m$ sequences according to the order $\leq_n$.
\begin{align*}
\class{(m,\varphi)} &= (m,\psi)\\
&\text{where $\psi\,i\,j = (\order_{m,j}\,\varphi)\,i\,j$}\\
\end{align*}
The key point to show the correctness of this algorithm is that 
...
We omit the proof, which is too technical and not very enlightening, believing that intuition drawn from the case of unordered pairs above is more interesting. 
